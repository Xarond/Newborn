...
bool WorldTile::isColliding(CollisionSet const& collisionSet) const {
  return Newborn::isColliding(collision, collisionSet);
}
...
bool ServerTile::isColliding(CollisionSet const& collisionSet) const {
  return Newborn::isColliding(getCollision(), collisionSet);
}
...
void ServerTile::read(DataStream& ds, VersionNumber serializationVersion) {
  if (serializationVersion < 416 || serializationVersion > CurrentSerializationVersion)
    throw NewbornException::format("Cannot read ServerTile - serialization version {} incompatible with current version {}\n", serializationVersion, CurrentSerializationVersion);
