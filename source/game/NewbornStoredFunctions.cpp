  ...
  if (targetValue < minValue) {
    ...
    while ((std::fabs(targetValue - value) > valueTolerance) && (searchDepth < 64)) {
      ...
      if (newValue > maxValue || newValue < minValue)
        throw NewbornException("StoredFunction is not monotonic.");
  ...
  for (auto& file : functions) {
    for (auto const& functionPair : assets->json(file).iterateObject()) {
      if (m_functions.contains(functionPair.first))
        throw NewbornException(strf("Named Function '{}' defined twice, second time from {}", functionPair.first, file));
    ...
    for (size_t y = 0; y < grid.size(); ++y) {
      ...
      if (y == 0) {
        ...
        if (cells.size() != xaxis.size())
          throw NewbornException("Number of sample points doesn't match axis size.");
