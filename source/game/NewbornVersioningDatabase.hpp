...
NEWBORN_STRUCT(VersionedJson);
NEWBORN_CLASS(VersioningDatabase);

NEWBORN_EXCEPTION(VersionedJsonException, NewbornException);
NEWBORN_EXCEPTION(VersioningDatabaseException, NewbornException);
...
struct VersionedJson {
  ...
  // header marking it as a newborn versioned json file.  Writes using a
