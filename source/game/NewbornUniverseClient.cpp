...
#include "NewbornSystemWorldClient.hpp"
...
void UniverseClient::setMainPlayer(PlayerPtr player) {
  if (isConnected())
    throw NewbornException("Cannot call UniverseClient::setMainPlayer while connected");
...
Maybe<String> UniverseClient::connect(UniverseConnection connection, bool allowAssetsMismatch, String const& account, String const& password) {
  ...
  if (!m_mainPlayer)
    throw NewbornException("Cannot call UniverseClient::connect with no main player");
  ...
  if (auto challenge = as<HandshakeChallengePacket>(packet)) {
    ...
    ByteArray passHash = Newborn::sha256(passAccountSalt);
