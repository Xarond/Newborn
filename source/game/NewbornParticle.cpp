...
void Particle::update(float dt, Vec2F const& wind) {
  ...
  if (ignoreWind) {
    velocity[0] = Newborn::approach<float>(finalVelocity[0], velocity[0], approach[0] * dt);
    velocity[1] = Newborn::approach<float>(finalVelocity[1], velocity[1], approach[1] * dt);
  } else {
    velocity[0] = Newborn::approach<float>(finalVelocity[0] + wind[0], velocity[0], approach[0] * dt);
    velocity[1] = Newborn::approach<float>(finalVelocity[1] + wind[1], velocity[1], approach[1] * dt);
  }
