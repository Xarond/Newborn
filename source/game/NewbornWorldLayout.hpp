...
NEWBORN_STRUCT(TerrainSelector);
NEWBORN_STRUCT(WorldRegion);
NEWBORN_CLASS(WorldLayout);
...
inline BiomeConstPtr const& WorldLayout::getBiome(BiomeIndex index) const {
  if (index == NullBiomeIndex || index > m_biomes.size())
    throw NewbornException("WorldLayout::getTerrainSelector called with null or out of range BiomeIndex");
...
inline TerrainSelectorConstPtr const& WorldLayout::getTerrainSelector(TerrainSelectorIndex index) const {
  if (index == NullBiomeIndex || index > m_terrainSelectors.size())
    throw NewbornException("WorldLayout::getTerrainSelector called with null or out of range TerrainSelectorIndex");
