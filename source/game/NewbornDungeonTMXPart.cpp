...
#include "NewbornDungeonTMXPart.hpp"
...
namespace Dungeon {
  ...
  bool TMXObject::forEachTile(TMXMap const* map, TileCallback const& callback) const {
    ...
    newbornAssert(m_kind == ObjectKind::Polyline);
  ...
  TMXTileLayer::TMXTileLayer(Json const& layer) {
    ...
    if (layer.optString("compression") == String("zlib")) {
      ...
      throw NewbornException::format("TMXTileLayer does not support compression mode {}", layer.getString("compression"));
    }

    if (m_tileData.count() != width * height)
      throw NewbornException("TMXTileLayer data length was inconsistent with width/height");
  }

  TMXMap::TMXMap(Json const& tmx) {
    if (tmx.getUInt("tileheight") != 8 || tmx.getUInt("tilewidth") != 8)
      throw NewbornException("Invalid tile size");
    ...
    for (Json const& tmxLayer : tmx.get("layers").iterateArray()) {
      ...
      if (layerType == "tilelayer") {
        ...
        throw NewbornException(strf("Unknown layer type '{}'", layerType.utf8Ptr()));
      }
    }
  }
  ...
  TMXTilesets::TMXTilesets(Json const& tmx) {
    for (Json const& tilesetJson : tmx.iterateArray()) {
      if (!tilesetJson.contains("source"))
        throw NewbornException::format("Tiled map has embedded tileset {}", tilesetJson.optString("name"));
  ...
  NewbornException TMXObject::tmxObjectError(Json const& tmx, String const& msg) {
    Vec2I pos = getPos(tmx);
    return NewbornException::format("At {},{}: {}", pos[0], pos[1], msg);
  }
