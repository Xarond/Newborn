...
SpeciesDatabase::SpeciesDatabase() {
  ...
  for (auto& file : files) {
    ...
    if (m_species.contains(speciesDefinition->kind()))
      throw NewbornException(strf("Duplicate species asset with kind {}. configfile {}", speciesDefinition->kind(), file));
...
SpeciesDefinitionPtr SpeciesDatabase::species(String const& kind) const {
  ...
  if (!m_species.contains(k))
    throw NewbornException(strf("Unknown species kind '{}'.", kind));
