...
NEWBORN_EXCEPTION(DungeonException, NewbornException);
...
public:
  ...
  virtual void placeObject(Vec2I const& pos, String const& objectName, Newborn::Direction direction, Json const& parameters) = 0;
...
namespace Dungeon {
  NEWBORN_CLASS(DungeonGeneratorWriter);
  NEWBORN_CLASS(PartReader);
  NEWBORN_CLASS(Part);
  NEWBORN_CLASS(Rule);
  NEWBORN_CLASS(Brush);
  NEWBORN_STRUCT(Tile);
  NEWBORN_CLASS(Connector);
  ...
  public:
    ObjectBrush(String const& object, Newborn::Direction direction, Json const& parameters);

    virtual void paint(Vec2I position, Phase phase, DungeonGeneratorWriter* writer) const override;

  private:
    String m_object;
    Newborn::Direction m_direction;
    Json m_parameters;
  };
