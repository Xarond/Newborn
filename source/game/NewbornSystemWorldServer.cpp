#include "NewbornSystemWorldServer.hpp"
#include "NewbornRoot.hpp"
#include "NewbornCelestialDatabase.hpp"
#include "NewbornCelestialGraphics.hpp"
...
void SystemWorldServer::removeObject(Uuid objectUuid) {
  if (!m_objects.contains(objectUuid))
    throw NewbornException(strf("Cannot remove object with uuid '{}', object doesn't exist.", objectUuid.hex()));

  if (m_objects[objectUuid]->permanent())
    throw NewbornException(strf("Cannot remove object with uuid '{}', object is marked permanent", objectUuid.hex()));
