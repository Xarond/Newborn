...
#include "NewbornMaterialRenderProfile.hpp"
#include "NewbornLiquidTypes.hpp"
#include "NewbornDamageDatabase.hpp"
#include "NewbornParticleDatabase.hpp"
#include "NewbornParticleManager.hpp"
#include "NewbornWorldImpl.hpp"
#include "NewbornPlayer.hpp"
#include "NewbornPlayerLog.hpp"
#include "NewbornAggressiveEntity.hpp"
...
void WorldClient::forEachCollisionBlock(RectI const& region, function<void(CollisionBlock const&)> const& iterator) const {
  ...
  m_tileArray->tileEach(region, [iterator](Vec2I const& pos, ClientTile const& tile) {
      if (tile.getCollision() == CollisionKind::Null) {
        ...
        newbornAssert(!tile.collisionCacheDirty);
  ...
  m_entityMap->forAllEntities([&](EntityPtr const& entity) {
      ...
      try { entity->render(&renderCallback); }
        ...
        if (entity->isMaster()) // this is YOUR problem!!
          ...
          String image = toolUser ? strf("/rendering/sprites/error_{}.png", DirectionNames.getRight(toolUser->facingDirection())) : "/rendering/error.png";
...
RpcPromise<InteractAction> WorldClient::interact(InteractRequest const& request) {
  ...
  if (auto targetEntity = m_entityMap->entity(request.targetId)) {
    if (targetEntity->isMaster()) {
      ...
      newbornAssert(interactiveTarget);
