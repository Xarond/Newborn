    ...
    if (auto key = p.second.second.maybe<String>()) {
      ...
      if (p.second.first == NodeParameterType::Vec2) {
        if (value.type() != Json::Type::Array)
          throw NewbornException(strf("Vec2 parameter not of array type for key {}", p.first, value));
...
NodeStatus BehaviorState::runNode(BehaviorNode const& node, NodeState& state) {
  ...
  if (node.is<ActionNode>())
    ...
    NewbornException("Unidentified behavior node type");
...
NodeStatus BehaviorState::runAction(ActionNode const& node, NodeState& state) {
  ...
  if (state.isNothing()) {
    ...
    try {
      ...
      throw NewbornException(strf("Lua Exception caught running action node {} in behavior {}: {}", node.name, m_tree->name, outputException(e, false)));
    }
    ...
    try {
      ...
      throw NewbornException(strf("Lua Exception caught resuming action node {} in behavior {}: {}", node.name, m_tree->name, outputException(e, false)));
    }
...
NodeStatus BehaviorState::runDecorator(DecoratorNode const& node, NodeState& state) {
  ...
  if (state.isNothing()) {
    ...
    try {
      ...
      throw NewbornException(strf("Lua Exception caught initializing decorator node {} in behavior {}: {}", node.name, m_tree->name, outputException(e, false)));
    }
    if (status == NodeStatus::Success || status == NodeStatus::Failure)
      return status;
  ...
  while (status == NodeStatus::Running) {
    ...
    if (childStatus == NodeStatus::Success || childStatus == NodeStatus::Failure) {
      try {
        ...
        throw NewbornException(strf("Lua Exception caught resuming decorator node {} in behavior {}: {}", node.name, m_tree->name, outputException(e, false)));
      }
    } else {
...
NodeStatus BehaviorState::runComposite(CompositeNode const& node, NodeState& state) {
  ...
  if (node.is<SequenceNode>())
    ...
    throw NewbornException(strf("Unable to run composite node type with variant type index {}", node.typeIndex()));
