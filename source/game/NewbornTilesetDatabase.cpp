...
namespace Tiled {
  ...
  BrushConstPtr getMaterialBrush(String const& materialName, Tiled::Properties& properties) {
    ...
    if (layer == TileLayer::Background) {
      ...
      newbornAssert(layer == TileLayer::Foreground);
  ...
  BrushConstPtr getObjectBrush(String const& objectName, Tiled::Properties& properties) {
    Newborn::Direction direction = Newborn::Direction::Right;
    Json parameters;
  ...
  size_t Tileset::size() const {
    newbornAssert(m_tilesBack.size() == m_tilesFront.size());
...
namespace Tiled {
  Json PropertyConverter<Json>::to(String const& propertyValue) {
    try {
      ...
      throw NewbornException::format("Error parsing Tiled property as Json: {}", outputException(e, false));
    }
  }
