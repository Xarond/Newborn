...
#include "NewbornMemoryAssetSource.hpp"
...
Assets::Assets(Settings settings, StringList assetSources) {
    ...
    auto decorateLuaContext = [&](LuaContext& context, MemoryAssetSourcePtr newFiles) {
        if (newFiles) {
            // re-add assets callbacks with more functions;
...
void Assets::workerMain() {
    while (true) {
        ...
        // After processing an asset, unlock the main asset mutex and yield so i
