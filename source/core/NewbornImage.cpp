...
void Image::reset(unsigned width, unsigned height, Maybe<PixelFormat> pf) {
  ...
  size_t imageSize = width * height * Newborn::bytesPerPixel(*pf);
  if (imageSize == 0) {
    if (m_data) {
      Newborn::free(m_data);
      m_data = nullptr;
    ...
    if (!m_data)
      newData = (uint8_t*)Newborn::malloc(imageSize);
    else
      newData = (uint8_t*)Newborn::realloc(m_data, imageSize);
...
Vec4B Image::clamp(Vec2I const& pos) const {
  Vec4B c;
  unsigned x = (unsigned)Newborn::clamp<int>(pos[0], 0, m_width - 1);
  unsigned y = (unsigned)Newborn::clamp<int>(pos[1], 0, m_height - 1);
