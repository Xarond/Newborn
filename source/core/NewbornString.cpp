...
StringList String::split(String const& pattern, size_t maxSplit) const {
  ...
  newbornAssert(maxSplit == NPos || ret.size() <= maxSplit + 1);
  return ret;
}
...
fmt::appender fmt::formatter<Newborn::String>::format(Newborn::String const& s, format_context& ctx) const {
