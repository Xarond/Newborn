...
#include "NewbornAudio.hpp"
#include "NewbornBuffer.hpp"
#include "NewbornIODeviceCallbacks.hpp"
...
public:
  ...
  void seekTime(double time) {
    ...
    if (ret != 0)
      throw NewbornException("Cannot seek ogg stream Audio::seekTime");
  }
  ...
  void seekSample(uint64_t pos) {
    ...
    if (ret != 0)
      throw NewbornException("Cannot seek ogg stream in Audio::seekSample");
  }
      ...
      for (unsigned destinationChannel = 0; destinationChannel < destinationChannels; ++destinationChannel) {
        ...
        for (int superSample = 0; superSample < SuperSampleFactor; ++superSample) {
          ...
          if (sourceSample < readSamples) {
            ...
            newbornAssert(sourceBufferIndex + sourceChannel < sourceBufferSize);
