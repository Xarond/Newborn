...
NEWBORN_EXCEPTION(MultiArrayException, NewbornException);
...
size_t MultiArray<Element, Rank>::storageIndex(IndexArray const& index) const {
  size_t loc = index[0];
  newbornAssert(index[0] < m_shape[0]);
  for (size_t i = 1; i < Rank; ++i) {
    loc = loc * m_shape[i] + index[i];
    newbornAssert(index[i] < m_shape[i]);
  }
  return loc;
}
...
template <typename Element, size_t Rank>
struct fmt::formatter<Newborn::MultiArray<Element, Rank>> : ostream_formatter {};
