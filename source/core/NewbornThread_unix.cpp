...
struct ThreadImpl {
  ...
  bool start() {
    ...
    if (ret != 0) {
      stopped = true;
      joined = true;
      throw NewbornException(strf("Failed to create thread, error {}", ret));
    }
...
#elif defined(NEWBORN_SYSTEM_NETBSD)
    pthread_setname_np(pthread, "%s", tname);
#elif not defined NEWBORN_SYSTEM_MACOS
  ...
  bool join() {
    ...
    if (ret != 0)
      throw NewbornException(strf("Failed to join thread, error {}", ret));
    joined = true;
    return true;
  }
...
unsigned Thread::numberOfProcessors() {
  ...
  if (nprocs < 1)
    throw NewbornException(strf("Could not determine number of CPUs online: {}\n", strerror(errno)));
  return nprocs;
}
