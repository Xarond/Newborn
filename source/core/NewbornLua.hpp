...
NEWBORN_EXCEPTION(LuaException, NewbornException);
...
// repl does.
NEWBORN_EXCEPTION(LuaIncompleteStatementException, LuaException);

// Thrown when the instruction limit is reached, if the instruction limit is
// set.
NEWBORN_EXCEPTION(LuaInstructionLimitReached, LuaException);

// Thrown when the engine recursion limit is reached, if the recursion limit is
// set.
NEWBORN_EXCEPTION(LuaRecursionLimitReached, LuaException);

// Thrown when an incorrect lua type is passed to something in C++ expecting a
// different type.
NEWBORN_EXCEPTION(LuaConversionException, LuaException);
...
template <> struct fmt::formatter<Newborn::LuaValue> : ostream_formatter {};
