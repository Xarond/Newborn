...
#ifdef NEWBORN_SYSTEM_MACOSX
#include <mach-o/dyld.h>
#elif defined NEWBORN_SYSTEM_FREEBSD
#include <sys/types.h>
...
String File::temporaryFileName() {
  return relativeTo(P_tmpdir, strf("newborn.tmpfile.{}", hexEncode(Random::randBytes(16))));
}
...
FilePtr File::ephemeralFile() {
  ...
  ByteArray path = ByteArray::fromCStringWithNull(relativeTo(P_tmpdir, "newborn.tmpfile.XXXXXXXX").utf8Ptr());
...
String File::temporaryDirectory() {
  String dirname = relativeTo(P_tmpdir, strf("newborn.tmpdir.{}", hexEncode(Random::randBytes(16))));
