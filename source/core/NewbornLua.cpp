...
LuaEngine::~LuaEngine() {
  ...
  newbornAssert(lua_gettop(m_state) == 0);
  lua_close(m_state);
}
...
void LuaEngine::countHook(lua_State* state, lua_Debug* ar) {
  newbornAssert(ar->event == LUA_HOOKCOUNT);
...
void* LuaEngine::allocate(void*, void* ptr, size_t oldSize, size_t newSize) {
  if (newSize == 0) {
    Newborn::free(ptr, oldSize);
    return nullptr;
  } else {
    return Newborn::realloc(ptr, newSize);
  }
}
...
LuaValue LuaEngine::popLuaValue(lua_State* state) {
  ...
  LuaValue result;
  newbornAssert(!lua_isnone(state, -1));
...
void LuaEngine::decrementRecursionLevel() {
  newbornAssert(m_recursionLevel != 0);
  --m_recursionLevel;
}
...
Maybe<LuaInt> LuaDetail::asInteger(LuaValue const& v) {
  ...
  /*// Kae: This prevents 1-1 conversion between Lua and Newborn::Json.
  if (v.is<LuaString>())
