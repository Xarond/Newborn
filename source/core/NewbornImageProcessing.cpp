...
StringList colorDirectivesFromConfig(JsonArray const& directives) {
  ...
  for (auto entry : directives) {
    if (entry.type() == Json::Type::String) {
      ...
      throw NewbornException("Malformed color directives list.");
    }
  }
  return result;
}
...
void processImageOperation(ImageOperation const& operation, Image& image, ImageReferenceCallback refCallback) {
  ...
  if (auto op = operation.ptr<HueShiftImageOperation>()) {
    ...
    if (!refCallback)
      throw NewbornException("Missing image ref callback during AlphaMaskImageOperation in ImageProcessor::process");
    ...
    if (!refCallback)
      throw NewbornException("Missing image ref callback during BlendImageOperation in ImageProcessor::process");
