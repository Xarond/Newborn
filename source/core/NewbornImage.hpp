...
NEWBORN_EXCEPTION(ImageException, NewbornException);
...
inline uint8_t Image::bitsPerPixel() const {
  return Newborn::bitsPerPixel(m_pixelFormat);
}

inline uint8_t Image::bytesPerPixel() const {
  return Newborn::bytesPerPixel(m_pixelFormat);
}
...
inline void Image::set32(unsigned x, unsigned y, Vec4B const& c) {
  newbornAssert(m_data && x < m_width && y < m_height);
  newbornAssert(bytesPerPixel() == 4);
