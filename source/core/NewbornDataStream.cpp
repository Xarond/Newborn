...
size_t DataStream::writeVlqU(uint64_t i) {
  return Newborn::writeVlqU(i, makeFunctionOutputIterator([this](uint8_t b) { *this << b; }));
}

size_t DataStream::writeVlqI(int64_t i) {
  return Newborn::writeVlqI(i, makeFunctionOutputIterator([this](uint8_t b) { *this << b; }));
}
...
size_t DataStream::readVlqU(uint64_t& i) {
  size_t bytesRead = Newborn::readVlqU(i, makeFunctionInputIterator([this]() { return this->read<uint8_t>(); }));
...
size_t DataStream::readVlqI(int64_t& i) {
  size_t bytesRead = Newborn::readVlqI(i, makeFunctionInputIterator([this]() { return this->read<uint8_t>(); }));
