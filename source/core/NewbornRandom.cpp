...
int64_t RandomSource::randInt(int64_t min, int64_t max) {
  if (max < min)
    throw NewbornException("Maximum bound in randInt must be >= minimum bound!");
  return randInt(max - min) + min;
}

uint64_t RandomSource::randUInt(uint64_t min, uint64_t max) {
  if (max < min)
    throw NewbornException("Maximum bound in randUInt must be >= minimum bound!");
  return randUInt(max - min) + min;
}

float RandomSource::randf(float min, float max) {
  if (max < min)
    throw NewbornException("Maximum bound in randf must be >= minimum bound!");
  return randf() * (max - min) + min;
}

double RandomSource::randd(double min, double max) {
  if (max < min)
    throw NewbornException("Maximum bound in randd must be >= minimum bound!");
