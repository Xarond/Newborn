#include "NewbornKeyBindings.hpp"
...
KeyChord inputDescriptorFromJson(Json const& json) {
  ...
  if (type == "key") {
    ...
    if (value.isType(Json::Type::String)) {
      ...
      throw NewbornException::format("Improper key value '{}'", value);
    }
  } else {
    throw NewbornException::format("Improper bindings type '{}'", type);
  }
...
KeyBindings::KeyBindings(Json const& json) {
  ...
  try {
    ...
    throw NewbornException(strf("Could not set keybindings from configuration. {}", outputException(e, false)));
  }
}
