...
LuaCallbacks LuaBindings::makeRenderingCallbacks(ClientApplication* app) {
  ...
  // if the last argument is defined and true, this change will also be saved to newborn.config and read on next game start, use for things such as an interface that does this
